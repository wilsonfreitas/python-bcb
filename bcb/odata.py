...
class ODataEntitySet(metaclass=ODataEntitySetMeta):
    ...
    def describe(self):
        props = ', '.join([
            ...
            for prop in self.entity.properties.values()
        ])
        print(f'''
...
class ODataMetadata:
    ...
    def _parse_entity(self, entity_element, namespace):
        ...
        props = {
            prop.attrib['Name']: ODataProperty(**prop.attrib)
            for prop in entity_element
        }
    ...
    def _parse_entity_sets(self, schema):
        ...
        def _parse_entity_set(e):
            ...
            return ODataEntitySet(e.attrib['Name'], e.attrib['EntityType'],
                                  entity)
        ...
        self._entity_sets_fullnames = {
            es.fullname: es
            for es in self.entity_sets.values()
        }
...
class ODataQuery:
    ...
    def collect(self):
        ...
        res = httpx.get(self.odata_url() + '?' + qs, headers=headers,
                        timeout=60.0)
    ...
    def show(self):
        ...
        if self.is_function:
            ...
            names = [
                f'{p.name}<{p.ftype}>'
                for p in self.entity.entity.properties.values()
            ]
            names = ', '.join(names)
            print('Return:', names)
